...
const static timer_square squares[] = {
    {.color = CRGB::Yellow, .start_ms = MIN_IN_MS(4), .end_ms = MIN_IN_MS(5), .end_notify_ms = MIN_IN_MS(4) + SEC_IN_MS(30)},
    {.color = CRGB::Yellow, .start_ms = MIN_IN_MS(3), .end_ms = MIN_IN_MS(4)},
    {.color = CRGB::Green, .start_ms = 0, .end_ms = MIN_IN_MS(1)},
    {.color = CRGB::Green, .start_ms = MIN_IN_MS(1), .end_ms = MIN_IN_MS(2)},
    {.color = CRGB::Green, .start_ms = MIN_IN_MS(2), .end_ms = MIN_IN_MS(3), .end_notify_ms = MIN_IN_MS(2) + SEC_IN_MS(30)}};
...
void draw_mode_countdown()
{
  ...
  { // 125 FPS
    ...
    for (i = 0; i < NUM_LEDS; i++)
    {
      ...
      {
        fract8 fade = (256 * (time - squares[i].start_ms)) / (squares[i].end_ms - squares[i].start_ms);
        leds[i].fadeToBlackBy(lerp8by8(0, 255, fade));

        if (squares[i].end_notify_ms && time >= squares[i].end_notify_ms)
        {
          fract8 end_fade = (256 * (time - squares[i].end_notify_ms)) / (squares[i].end_ms - squares[i].end_notify_ms);
...
void draw_mode_config()
{
  ...
  EVERY_N_MILLIS(8)
  {
    leds[0] = CHSV(0, 0, (config_mode == cfg_pattern && cursor_blink) ? 128 : 32);
    leds[1] = CHSV(0, 0, (config_mode == cfg_brightness && cursor_blink) ? 128 : 32);
    if (config_mode == cfg_brightness)
    {
      leds[2] = CHSV(0, 0, 85);
      leds[3] = CHSV(0, 0, 170);
